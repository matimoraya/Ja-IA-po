...
export class MockWorkplace implements Workplace {
  private tasks: Map<string, WorkplaceTask> = new Map();
  ...
  async get(id: string) {
    ...
    if (!task) throw new FollowupError("Task not found in mock store.");
    return task;
  }

  async list(marker: string) {
    return Array.from(this.tasks.values()).filter((t) => t.description?.includes(marker) || t.title.includes(marker));
  }
}
...
export function configuredWorkplace(apiKey = process.env.AMBIGUOUS_API_KEY): {
  ...
  if (!apiKey?.trim()) {
    console.log("No AMBIGUOUS_API_KEY provided. Using MockWorkplace.");
    return {
      workplace: new MockWorkplace(),
      close: async () => {},
    };
  }
